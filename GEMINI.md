...
- **同步点**: 2026-04-08 (当前进度)
  - [x] **全站精英视觉 (Elite UI)**: 确立了 **Cormorant Garamond** 字体栈与 100vh 分屏叙事逻辑。
  - [x] **Elite Admin 深度重塑**: 彻底弃用了 UmiJS 的 `ProLayout` 插件，转为在 `src/layouts/index.tsx` 中手动实现高性能全局布局。
  - [x] **极致主题同步**: 解决了亮暗模式切换的实时响应问题，实现了全站色值、Favicon 及网页标题的统一对齐。
  - [x] **全栈规范升级**: 强制 Gemini CLI 在本项目的所有交互中全程使用 **中文**。
  - [x] **后台架构**: 解决了 `history` 导入报错、布局闪烁及 CSS 过渡卡顿等架构难题。
  - **待办 (Next Steps)**:
    1. **[后台] 菜单管理 (MVP)**: 实现侧边栏配置持久化，对接 `blogApi` 的菜单 CRUD 接口。
    2. **[后台] 文章管理 (编辑器)**: 接入 Markdown 编辑器 (如 Vditor 或 TipTap)，实现具备高奢质感的内容编辑体验。
    3. **[后端] 文章系统建模**: 设计并迁移 `Article` 与 `Category` 数据库模型，开发对应的增删改查 REST API。
