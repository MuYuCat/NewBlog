...
- **同步点**: 2026-04-16 (Elite 文章详情与物理视觉奇观)
  - [x] **内容详情闭环 (Article Detail)**: 基于 `ArticleDetail.vue` 实现了文章的获取、阅读量统计、以及基于 `markdown-it` 的全栈深度排版渲染。
  - [x] **物理视觉奇观 (文字坠落成海)**:
    - **高性能内核**: 成功集成了 `@chenglou/pretext` 文本测量库，实现了 0 Reflow 的极速字符位置推算，规避了长文布局时的性能崩溃。
    - **Matter.js 仿真**: 引入物理引擎，实现了字符的自由落体与触底浮力模拟，打造了“坠落成海”的视觉奇观。
    - **交互恢复**: 实现了静置触发与实时活动感知的双态无缝切换。
  - **待办 (Next Steps)**:
    1. **[后台] 发射中心补全**: 在 `apps/blogAdmin/src/pages/article/edit.tsx` 中完善支持 Word (mammoth.js) 协议转换的高级编辑器。
    2. **[移动端] 跨端启动**: 启动 `blogMobile` (uni-app) 的实质性页面构建。
    3. **[前台] 体验微调**: 针对坠落特效进行进一步的性能压力测试（尤其是 10 万字以上的极端文本）。
