...
## 2026-04-14：内容空间枢纽 (Article Hub) 架构进化与 UI/UX 深度重塑

### 🚀 重大变更

- **数据库模型重构 (Prisma Refactor)**:
  - **扁平化去冗余**: 彻底移除了 `Article` 模型中的 `type` 和 `slug` 字段，回归极简内容核心。
  - **多维关联升级 (M:N)**: 将分类关联由 1:N 升级为 M:N (多对多)。现在一篇文章支持关联多个“主题维度”，实现了更灵活的内容组织。
  - **后端健壮性**: 在 `ArticleService` 中实现了局部字段更新逻辑，确保在切换状态或修改属性时不会意外清空已有的维度或标签关联。
- **Elite UI 交互革命 (UX Optimization)**:
  - **管理列交互对齐**:
    - **按钮常驻**: 废弃了 Hover 触发逻辑，编辑、删除按钮默认常驻显示，操作尺寸提升至 44x44px（符合 Apple HIG）。
    - **快速路径**: 引入了“一键发布 (Rocket)”与“可见性切换 (Eye)”图标按钮，替代了笨重的 Switch 滑块，实现了 100% 的视觉语言对齐。
  - **列表视觉精修**:
    - **统一标签规范**: 维度标签与状态标签统一采用 8px 圆角及半透明磨砂背景（10% 不透明度），建立了全站统一的 Elite Tag 体系。
    - **时间逻辑修正**: 将列表展示时间由“创建时间”修改为“更新时间”，并适配了后端 `updatedAt: 'desc'` 排序，确保活跃内容始终置顶。
    - **排版分层**: 优化了简介（Summary）的排版，引入 `line-height: 1.6` 和双行截断，提升了长列表的扫描效率。
- **全栈编辑体验增强**:
  - **多选维度支持**: 文章编辑页侧边栏升级为多选模式，支持实时回显与同步。
  - **状态云端联动**: 在编辑页侧边栏引入了“前台展示”开关，支持在发布前精准控制内容的可见性。
- **像素级布局修复**:
  - **搜索栏对齐**: 深度修复了多选 Select 在 `borderless` 模式下的垂直居中难题，通过控制 `.ant-select-selection-wrap` 实现了像素级对齐。
  - **分页器一致性**: 统一了页码按钮与“条/页”切换器的物理尺寸（38px）与圆角风格。

### 📦 交付物

- `apps/blogApi/prisma/schema.prisma`: 支持 M:N 关联的新一代内容模型。
- `apps/blogAdmin/src/pages/article/index.tsx`: 具备快速发射与多维索引能力的文章管理中心。
- `apps/blogAdmin/src/pages/article/article.scss`: 统一了 Elite Tag 与 Elite Button 的高奢视觉样式表。
- `apps/blogApi/src/article/`: 支持局部更新与多维查询的高性能后端服务。
