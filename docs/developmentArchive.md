...
## 2026-04-17：全栈构建加固与 Game Hub 逻辑补全

### 🚀 重大变更

- **全栈稳定构建 (Stable Build)**:
  - **依赖修复 (Admin)**: 修复了 `blogAdmin` 中 `@ant-design/pro-components` 符号链接断裂导致的构建失败。通过全栈 `pnpm install` 重新校准了依赖拓扑。
  - **原生模块权限 (Root)**: 在根目录 `package.json` 中配置了 `pnpm.onlyBuiltDependencies` 白名单，允许 `sharp`、`prisma`、`esbuild` 等原生依赖执行编译脚本，解决了 Astro 图片优化及 NestJS 数据库驱动失效问题。
  - **Astro 视觉补完 (Web)**: 为 `blogWeb` 显式引入 `sharp` 依赖，确保了 404 页面及 Bento Grid 等核心板块的高性能图片预处理。
- **Game Hub 逻辑闭环**:
  - **配置持久化 (API)**: 补全了 `GameService.updateSyncConfig` 方法，实现了基于 `upsert` 协议的平台同步凭证安全更新。
  - **类型系统对齐**: 解决了 NestJS 环境下 `cron` 类型定义冲突，移除了冗余的 `@types/cron`，转而使用 `cron` 原生类型。

### 🛠 技术修复

- **物理同步锁稳定性**: 验证了环境感知锁在多应用构建场景下的原子性，确保了同步指令的单例执行。
- **TS 类型收敛**: 修复了 `blogApi` 中多处因模型重构导致的残余类型报错。

---

## 2026-04-17：游戏轨迹地基竣工与全栈“安全锁”体系

### 🚀 重大变更

- **游戏轨迹 (Game Hub) 架构重构**:
  - **Master-Platform 模型**: 弃用扁平结构，引入“主游戏实体 (Master) - 游玩平台 (Platform) - 成就 (Achievement)”三级架构，支持同款游戏跨平台数据聚合。
  - **同步配置系统**: 建立 `GameSyncConfig` 模型，实现敏感凭证的加密存储与同步状态的实时持久化。
- **全栈自动化安全阀 (Safety Valve)**:
  - **双重冷却机制**: 实现 12 小时自动同步 (Cron) 与 2 小时手动同步 CD。
  - **环境感知锁定**: 物理打通 `DEV` 与 `PROD` 环境，公用同一数据库锁，防止本地调试导致的高频误伤。
  - **智能风险熔断**: 一旦检测到 `403 Forbidden` 等风控信号，系统自动进入 `RISK` 锁定状态，物理切断一切自动尝试，保全账号资产。
  - **差异化冷却**: 成功同步进入长 CD（2小时），失败同步仅进入短 CD（5分钟），优化重试体验。
- **管理后台 (Game Lab) 1.0**:
  - **监控仪表盘**: 实现了具备实时倒计时与平台健康度感知的同步控制台。
  - **档案中心**: 构建了支持搜索、状态过滤与跨平台标记的游戏库管理界面。

### 🛠 技术修复

- **TS 声明修复**: 解决了 `blogWeb` 中 Vue 组件类型丢失导致的编译崩溃。
- **布局重叠修复**: 采用 Grid 布局彻底解决了文章详情页长内容遮挡侧边目录的问题。
- **编辑器协议增强**: 实现了 Word 剪贴板“暴力清洗”算法，解决了粘贴时的样式污染与双份内容难题。
